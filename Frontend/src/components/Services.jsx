              ...
              <div className="w-full max-w-4xl bg-white/5 backdrop-blur-md rounded-3xl p-8 shadow-2xl border border-white/10">
                <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 gap-4">
                  ...
                  <div className="flex gap-3 flex-wrap">
                    <motion.button onClick={handleDownloadPdf} disabled={!pdfUrl} className="bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 disabled:opacity-50 text-white px-6 py-3 rounded-2xl font-medium" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                      <span>⬇️</span> Download PDF
                    </motion.button>
                    <motion.button onClick={handleSaveToLibrary} disabled={saveLoading || !pdfUrl} className="bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-700 hover:to-emerald-800 disabled:opacity-50 text-white px-6 py-3 rounded-2xl font-medium" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                      {saveLoading ? 'Saving...' : <><span>💾</span> Save to Library</>}
                    </motion.button>
                    <motion.button onClick={handleClosePdfPreview} className="bg-gradient-to-r from-gray-600 to-gray-700 hover:from-gray-700 hover:to-gray-800 text-white px-6 py-3 rounded-2xl font-medium" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                      <span>✕</span> Close
                    </motion.button>
                  </div>
                </div>
