...
export default function Signup({ isOpen, onClose }) {
  ...
  const getPasswordStrength = (password) => {
    ...
    if (password.length < 10) return { level: 2, text: 'Medium', color: 'text-yellow-400' };
      ...
      <div className="relative bg-white/5 backdrop-blur-md border border-white/10 rounded-3xl p-8 w-full max-w-md mx-4 shadow-2xl min-h-[400px] max-h-[90vh] overflow-y-auto flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full bg-gradient-to-r from-emerald-500 via-blue-500 to-purple-500 text-white py-3 px-6 rounded-xl font-semibold transition-all duration-200 hover:shadow-lg hover:shadow-emerald-500/25 hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed relative overflow-hidden"
          >
            {isLoading ? (
