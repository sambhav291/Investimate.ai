...
const Library = () => {
  ...
  useEffect(() => {
    const fetchReports = async () => {
      ...
      setError("");
      try {
        // ✅ **THE FIX IS HERE**: Instead of using the custom hook for this initial fetch,
        // we use the standard `fetch` API directly. This is crucial because it guarantees
        // we are using the absolute latest `token` value from the context, which prevents the race condition.
        ...
        if (!res.ok) {
          // If the request fails even with the latest token, then the session is truly invalid.
          // The AuthContext will automatically handle the logout via its own refresh logic.
          if (res.status === 401) {
    ...
    // This logic is perfect. It waits until authentication is confirmed before fetching.
    if (authStatus === 'authenticated' && token) {
      fetchReports();
    } else if (authStatus === 'unauthenticated') {
      // If the user is not logged in, clear state and stop loading.
      setLoading(false);
      setReports([]);
      setFilteredReports([]);
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [token, authStatus]); // Dependencies are correct.

  // --- ALL OTHER FUNCTIONS AND JSX IN YOUR FILE REMAIN EXACTLY THE SAME ---
  // The custom `fetchWithAuth` hook is still used for other actions like
  // opening, deleting, and previewing PDFs, which is perfectly fine as the
  // token will be stable by then.
  ...
  useEffect(() => {
    if (storagePath) {
      ...
      fetchWithAuth(`${API_ENDPOINTS.previewPdf}?storage_path=${encodeURIComponent(storagePath)}`)
        .then(res => res.blob())
        .then(blob => {
...
//       setError("");
//       try {
//         const res = await fetchWithAuth(API_ENDPOINTS.myReports);
//         if (!res.ok) throw new Error("Failed to fetch reports. Try logging in again.");
//         const data = await res.json();
//         setReports(data);
//         setFilteredReports(data);
//       } catch (err) {
