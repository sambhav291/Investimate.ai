...
// import { FileText, Download, Trash2, Eye, Clock, FolderOpen, Search, Filter } from "lucide-react";
...
//       setError("");
//       try {
//         // Use fetchWithAuth for seamless token refresh
//         const res = await fetchWithAuth("http://localhost:8000/my-reports");
...
//   useEffect(() => {
//   if (storagePath) {
//     fetch(`http://localhost:8000/preview-pdf?storage_path=${encodeURIComponent(storagePath)}`, {
//       credentials: "include",
//     })
//       .then(res => res.blob())
//       .then(blob => {
...
//   try {
//     const res = await fetchWithAuth(`http://localhost:8000/my-reports/${reportId}`, {
...
//     }
//     const data = await res.json();
//     setStoragePath(data.filepath); // Use storage path for preview
...
// // // In your modal or preview section:
// // {storagePath && (
// //   <iframe
// //     src={`http://localhost:8000/preview-pdf?storage_path=${encodeURIComponent(storagePath)}`}
...
//     try {
//       const res = await fetchWithAuth(`http://localhost:8000/delete-report/${reportId}`, {
//         method: "DELETE",
//         headers: { Authorization: `Bearer ${token}` },
//       });
//       if (res.ok) {
...
//             <p className="text-gray-600">Please login to view your library and access your reports.</p>
//           </div>
//         </div>
//       </div>
//     );
...
//           </div>
//           <p className="text-gray-600">Manage and view your saved reports</p>
//         </div>
...
//                 : "Your saved reports will appear here. Generate your first report to get started!"
//               }
//             </p>
//             {searchTerm && (
//               <button
...
//                 <div key={report.id} className="group bg-white rounded-2xl shadow-sm border border-gray-100 hover:shadow-lg hover:border-gray-200 transition-all duration-200 overflow-hidden">
//                   <div className="p-6">
//                     {/* File Icon */}
...
//                         className="flex-1 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2.5 rounded-xl font-medium transition-all duration-200 flex items-center justify-center gap-2 hover:scale-[1.02] active:scale-[0.98]"
//                       >
...
//                         className="bg-gray-100 hover:bg-red-50 text-gray-600 hover:text-red-600 p-2.5 rounded-xl transition-all duration-200 hover:scale-[1.02] active:scale-[0.98]"
//                       >
...
//                 </button>
//                 <a
//                   href={`http://localhost:8000/preview-pdf?storage_path=${encodeURIComponent(storagePath)}&download=1`}
//                   download
//                   className="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-medium transition-all duration-200 flex items-center gap-2"
//                 >
...
const Library = () => {
  ...
  // Fetch all reports
  useEffect(() => {
    // Only run when token or authStatus changes
    const fetchReports = async () => {
      setLoading(true);
      setError("");
      try {
        ...
        // Only set error if not session expired
  ...
  }, [token, authStatus]); // <-- fetchWithAuth removed from dependencies
  ...
  useEffect(() => {
    if (storagePath) {
      console.log("[PDF PREVIEW] storagePath set, fetching preview...");
      fetchWithAuth(`http://localhost:8000/preview-pdf?storage_path=${encodeURIComponent(storagePath)}`)
        .then(res => res.blob())
        .then(blob => {
          const url = URL.createObjectURL(blob);
          console.log("[PDF PREVIEW] Blob URL created:", url);
          setPdfBlobUrl(url);
        })
        .catch(err => {
      ...
      console.log("[PDF PREVIEW] storagePath cleared");
      setPdfBlobUrl(null);
    }
  }, [storagePath]);
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="hidden lg:flex gap-4">
              ...
              <motion.div
                ...
                className="bg-white/10 backdrop-blur-md  rounded-2xl py-3 px-4 text-center"
              >
                <div className="text-2xl font-bold text-green-400">
                  {reports.reduce((acc, report) => acc + (report.file_size || 0), 0) > 0 
                    ? formatFileSize(reports.reduce((acc, report) => acc + (report.file_size || 0), 0))
                    : "0 MB"
                  }
                </div>
...
//   // Fetch all reports
//   useEffect(() => {
//     console.log("[INIT] Library component mounted");
//     const fetchReports = async () => {
//       console.log("[fetchReports] Fetching reports...");
//       setLoading(true);
//       setError("");
//       try {
//         const res = await fetch("http://localhost:8000/my-reports", {
//           credentials: "include",
//         });
//         if (!res.ok) throw new Error("Failed to fetch dichuk reports");
//         const data = await res.json();
...
//       } finally {
//         setLoading(false);
//         console.log("[fetchReports] Loading state off");
//       }
//     };
//     if (token) {
//       console.log("[Auth] Token found, fetching reports...");
//       fetchReports();
//     } else {
//       console.warn("[Auth] No token, skipping fetch.");
//     }
//   }, [token]);
